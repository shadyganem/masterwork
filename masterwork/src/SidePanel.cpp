	...
	const wxString& name) : wxPanel(parent, winid, pos, size, style, name)
{
	...
	// Note the last param corresponds to the id given to the menu item
	this->Bind(wxEVT_MENU, &mw::SidePanel::OnMenuClick, this, wxID_DELETE);
...
void mw::SidePanel::OnProjectsTreeRightClick(wxMouseEvent& event)
{
	...
	{
		wxMenu contextMenu;
		contextMenu.Append(wxID_DELETE, "Deactivate");
		m_project_tree->PopupMenu(&contextMenu, pos);
	}
	event.Skip();
}

void mw::SidePanel::OnMenuClick(wxCommandEvent& event)
{
	mw::Controller& controller = mw::Controller::Get();
